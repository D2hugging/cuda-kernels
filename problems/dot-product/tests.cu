...
static float runDotProductWarpShuffle(const float *h_a, const float *h_b,
                                      size_t n) {
  float *d_a, *d_b, *d_partialSum, *d_res;
  int numBlocks = std::min(static_cast<int>((n + BLOCK_SIZE - 1) / BLOCK_SIZE),
                           MAX_GRID_BLOCKS);
  ...
  dotProductWarpShuffleKernel<<<numBlocks, BLOCK_SIZE>>>(d_a, d_b, d_partialSum,
                                                         n);
...
static bool testSingleElement() {
  ...
  float gpu_res = runDotProductWarpShuffle(a.data(), b.data(), 1);
...
static bool testAllZeros() {
  ...
  float gpu_res = runDotProductWarpShuffle(a.data(), b.data(), n);
